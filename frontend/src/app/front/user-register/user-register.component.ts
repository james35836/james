...
export class UserRegisterComponent implements OnInit {
	...
	load_school_year()
	{
		this.http.post(this.rest.domain+"/api/get_school_year", {}, 
		{

		})
		.subscribe(response =>
		{
			this.school_year_list = response;
			if(response != null)
			{
				this.add_member.school_year_id = this.school_year_list[0].school_year_id;
			}
		},
		error =>
		{
